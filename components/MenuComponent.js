...
const MenuComponent = ({ food }) => {
  ...
  return (
    <View>
      <View style={{ height: .7, backgroundColor: 'gray', width: '93%', alignSelf: 'center', marginVertical: 12 }} />
      <Pressable
        onPress={() => dispatch(toggleBottomSheet({ open: true, food }))}
        style={{
          margin: 7,
          flexDirection: "row",
      ...
      >
        <View>
          <Text style={{ fontSize: 13, fontWeight: "600" }}>{food.name}</Text>
          <Text style={{ fontSize: 12, fontWeight: "600" }}>{food.price}</Text>
          <Text
            style={{
              marginTop: 5,
              borderRadius: 4,
